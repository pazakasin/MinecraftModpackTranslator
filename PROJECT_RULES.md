...
- ✅ 読みやすい文体
- ✅ **コードブロックの正確な記述**
  - すべてのコードブロックは開始（```）と終了（```）を必ず対応させる
  - コードブロック間に説明文を挟む場合は、必ず一度閉じてから新しいブロックを開始
  - 言語指定を明確にする（java, markdown, bash等）
  - 連続する複数のコードは、それぞれ独立したブロックとして記述
...
| バージョン | 日付 | 変更内容 |
|-----------|------|----------|
| v2.11 | 2025-12-02 | Markdown記法ルール追加（セクション12） |
...
#### 基本ルール
- すべてのコードブロックは ``` で開始し、``` で終了する
- 開始と終了は必ず対応させる（開始したら必ず閉じる）
- コードブロック間に通常のテキストを挟む場合は、必ず前のブロックを閉じる
...
コード出力前に以下を確認：
1. ✅ すべての ``` に対応する閉じタグがあるか
2. ✅ コードブロック間の説明文が正しく表示されるか
3. ✅ 言語指定が適切か
4. ✅ インデントが崩れていないか

### 12.3 修正が必要な場合

出力後にMarkdownの表示が崩れている場合：
1. ユーザーに通知する
2. 正しいMarkdownで再出力する
